...
export default function MainContainer() {
    ...
    // 0 rep 2 start, 1 reenact, 2 blood mana, 3 near/far + rw, 4 idyllic 1, 5 platform break, 6 stacks/defs, 7 clone stacks, 8 idyllic 2, 9 hell 1, 10 hell 2
    ...
    const outputGeneric = [
        ...
        [`${platform}`, `${mitPlan[5]}`], // Which side to go for tower platforms; mit reminder
        [`mid`, `${instructions[1]} after`], // Reminder to go mid after tower assignments; sneak preview of your first S/D spot
        [`${instructions[1]}`, `burst`, `${mitPlan[6]}`], // First S/D (stack/def) spot; mit reminder
        [`${instructions[2]}`, `${mitPlan[6]}`], // Second S/D spot (non-healers can use same mit message for every hit)
        [`${instructions[3]}`, `${mitPlan[6]}`], // Third S/D spot
        [`${instructions[4]}`, `${mitPlan[6]}`], // Fourth S/D spot
        ...
        [`${tower[3]}`, `${tower[4]}`], // Where to go for cone spreads
        [`${cardsOrInters}`, `${mitPlan[7]}`], // Where to go for first clone stacks; mit reminder
        ...
        [`${portalClone}`, `${mitPlan[8]}`], // Where to go to dodge the final clone telegraph

        // Enrage
        [`tankbuster`],
        [`pot soon!`], // Pot reminder
        [`${mitPlan[9]}`], // Mits for Arcadian Hell 1
        [`${mitPlan[10]}`] // Mits for Arcadian Hell 2
    ]
    ...
    function renderContent() {
        ...
        switch (step) {
            ...
            case 40: return <Output messages={outputMessages[26]} timeout={12000} audio={[`/${cardsOrInters}.wav`]} />; // where to go for first stacks
            case 41: return <Output messages={outputMessages[27]} timeout={12000} audio={[`/${safePlatform[0]}.wav`, `/${safePlatform[1]}.wav`]} />; // safe platform
            case 42: return <Output messages={outputMessages[28]} timeout={10000} audio={[`/${cardsOrInters == "cards" ? ("intercards.wav") : ("cards.wav")}`]} />; // second stacks
            case 43: return <Output messages={outputMessages[29]} timeout={12000} audio={[`/${portalClone}.wav`]} />; // portal clone --> castbar damage
            case 44: return <Output messages={[` `]} timeout={2000} />; // just hit boss time
            case 45: return <Output messages={outputMessages[30]} timeout={10000} />; // audio={[`/tankbuster.wav`]}
            case 46: return <Output messages={outputMessages[31]} timeout={12000} audio={[`/pot soon.wav`]} />; // pot reminder for 8:00 burst
            case 47: return <Output messages={[` `]} timeout={10000} />; // just hit boss time
            case 48: return <Output messages={outputMessages[32]} timeout={12000} />; // arcadian hell 1
            case 49: return <Output messages={outputMessages[33]} timeout={13000} />; // arcadian hell 2
        }
    }
